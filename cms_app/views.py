...
def doLogin(request):
    ...
    login(request, user)
    print("User is: ")
    print(request.user) #request.user gives us the username
...
def doRegistration(request):
    ...
    if CustomUser.objects.filter(email=email_id).exists():
        messages.error(request, 'A user with this email already exists. Please proceed to login!')
        return render(request, 'registration.html')
    
    user_type = get_user_type_from_email(email_id)

    if user_type is None:
        messages.error(request, "Please use a valid format for the email id: '<username>.<staff|student|hod>@<college_domain>'")
        return render(request, 'registration.html')

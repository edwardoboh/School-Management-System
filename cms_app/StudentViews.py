...
def student_home(request):
    ...
    student_obj = Students.object.get(admin=request.user.id)
    total_attendance = AttendanceReport.objects.filter(student_id=student_obj).count()
    attendance_present = AttendanceReport.objects.filter(student_id=student_obj, status=True).count()
    attendance_absent = AttendanceReport.objects.filter(student_id=student_obj, status=False).count()
    course_obj = Courses.object.get(id=student_obj.course_id.id)
...
def student_view_attendance(request):
    ...
    subjects = Subjects.object.filter(course_id=course)
    context = {
        "subjects": subjects
    }
...
def student_leave_apply_save(request):
    if request.method != 'POST':
        messages.error(request, "Invalid Request Method")
        return redirect('student_template/student_apply_leave.html')
    else:
...
def student_feedback_save(request):
    if request.method != 'POST':
        ...
        try:
            ...
            add_feedback.save()
            messages.success(request, "Added feedback")
            return redirect("student_feedback")
        except:
...
def student_profile_update(request):
    if request.method != 'POST':
        ...
        try:
            ...
            customuser.firstname = first_name
